...
dataset = WaymoDataset('./data/tf_records/validation', './data/idxs/validation')
...
print('Input shape:', states_batch.shape, '[A,T,K] -- A agents, T time steps, K number of input features.')
...
print('Encoded shape:', encodings.shape, '[A,T,D] -- D the total number of model features')

decoding = decoder(encodings, agents_batch_mask, states_padding_mask_batch, 
                        states_hidden_mask_batch)

print('Output shape', decoding.shape, '[F,A,T,6] -- F features logits for classifying')
